...
def create_temporal_breakout_oos_study(csv_path, output_dir, *, lookback_bars=240, round_trip_cost_points=1.0, side_policy="BOTH", train_ratio=0.6, validation_ratio=0.2, oos_ratio=0.2) -> Path:
    """Publish three independent, chronological breakout replays without selection."""
    try:
        config = BreakoutReplayConfig(lookback_bars, round_trip_cost_points, side_policy=side_policy)
        ...
        configuration = {"lookback_bars":config.lookback_bars,"round_trip_cost_points":config.round_trip_cost_points,"side_policy":config.side_policy,"ratios":{"train":train_ratio,"validation":validation_ratio,"oos":oos_ratio},"execution":"next_bar_open","selection":"none"}
        ...
        summary = {"schema_version":"1.0","configuration":configuration,"segments":rows,"temporal_contract":{"ordered":["train","validation","oos"],"disjoint":True,"each_starts_flat":True,"warmup_crosses_boundaries":False,"oos_used_for_selection":False,"segment_boundary_index_basis":"source_global","trade_and_decision_index_basis":"segment_local"}}
        manifest = {"schema_version":"1.0","run_id":f"breakout-temporal-oos-{run_hash[:16]}","input_filename":source.name,"input_sha256":input_hash,"configuration_sha256":hashlib.sha256(deterministic_json(configuration).encode("utf-8")).hexdigest(),"status":"completed","generated_files":["manifest.json","results.json","summary.json"],"warnings":["segments are chronological, disjoint, and independently replayed from FLAT","open positions are closed with END_OF_DATA at each segment boundary","no automatic comparison or optimization is performed","the nine existing contracts are development data and are not an independent holdout"]}
