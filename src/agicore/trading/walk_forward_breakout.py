...
def create_walk_forward_breakout_study(csv_path, output_dir, *, initial_train_bars, validation_bars, oos_bars, lookback_bars=240, round_trip_cost_points=1.0, side_policy="BOTH"):
    try:
        ...
        config=BreakoutReplayConfig(lookback_bars,round_trip_cost_points,side_policy=side_policy); source,final=Path(csv_path).resolve(),Path(output_dir).resolve()
