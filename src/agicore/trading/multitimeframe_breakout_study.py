...
def create_multitimeframe_breakout_study(csv_paths, output_dir, round_trip_cost_points=1.0, side_policy="BOTH"):
    if round_trip_cost_points < 0: raise MultiTimeframeStudyError("round_trip_cost_points must be greater than or equal to 0")
    try: BreakoutReplayConfig(side_policy=side_policy)
    ...
    try:
        ...
        for source in paths:
            ...
            for timeframe,lookback in TIMEFRAMES.items():
                ...
                bars=source_bars if timeframe==1 else tuple(load_ohlcv_csv(target)); replay=replay_breakout(bars,BreakoutReplayConfig(lookback,round_trip_cost_points,side_policy=side_policy)); trades=replay["trades"]
        ...
        summary={"schema_version":"1.2","configuration":config,"multi_source_aggregation":aggregation_policy,"by_timeframe":{str(tf):_timeframe_aggregate((r for r in rows if r["timeframe_minutes"]==tf),tuple(trades_by_timeframe[tf])) for tf in TIMEFRAMES}}
        manifest={"schema_version":"1.2","run_id":f"multitimeframe-{run[:16]}","input_filenames":[p.name for p in paths],"configuration":config,"multi_source_aggregation":aggregation_policy,"status":"completed","generated_files":["manifest.json","report.md","results.json","summary.json"],"warnings":["descriptive multi-timeframe study; no automatic selection or optimization","multi-source metrics are path-dependent descriptive aggregates, not a rollover-adjusted continuous equity curve"]}
        report="# Multi-Timeframe Breakout Study\n\n"+"\n".join(["- Offline descriptive study; no timeframe is selected automatically.",f"- Inputs: {', '.join(p.name for p in paths)}",f"- Cost: {round_trip_cost_points:.2f}",f"- Side policy: {side_policy}"])+"\n"
