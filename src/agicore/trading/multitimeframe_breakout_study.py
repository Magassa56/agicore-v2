...
from .breakout_replay import BreakoutReplayConfig, BreakoutReplayError, replay_breakout
    ...
    try:
        ...
        for source in paths:
            for timeframe,lookback in TIMEFRAMES.items():
                ...
                bars=tuple(load_ohlcv_csv(target)); replay=replay_breakout(bars,BreakoutReplayConfig(lookback,round_trip_cost_points)); trades=replay["trades"]
                net=[t.net_pnl_points for t in trades]; longs=[t for t in trades if t.side=="LONG"]; shorts=[t for t in trades if t.side=="SHORT"]
                rows.append({"input_filename":source.name,"input_sha256":sha256_file(source),"timeframe_minutes":timeframe,"lookback_bars":lookback,"input_bar_count":len(load_ohlcv_csv(source)),"resampled_bar_count":len(bars),"dropped_incomplete_bucket_count":0 if timeframe==1 else None,"total_trades":len(trades),"net_total_pnl_points":sum(net),"net_profit_factor":sum(x for x in net if x>0)/abs(sum(x for x in net if x<0)) if any(x<0 for x in net) else None,"net_average_trade_points":sum(net)/len(net) if net else 0.0,"net_closed_equity_drawdown_points":0.0,"long_trades":len(longs),"long_net_pnl_points":sum(t.net_pnl_points for t in longs),"short_trades":len(shorts),"short_net_pnl_points":sum(t.net_pnl_points for t in shorts),"first_market_timestamp":bars[0].timestamp.isoformat(),"last_market_timestamp":bars[-1].timestamp.isoformat(),"run_id":hashlib.sha256(f"{sha256_file(source)}:{timeframe}:{lookback}:{round_trip_cost_points}".encode()).hexdigest()[:20]})
        config={"timeframes":TIMEFRAMES,"round_trip_cost_points":round_trip_cost_points}; run=hashlib.sha256(json.dumps({"inputs":[sha256_file(p) for p in paths],**config},sort_keys=True).encode()).hexdigest()
