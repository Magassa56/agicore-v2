...
def create_multitimeframe_breakout_study(csv_paths, output_dir, round_trip_cost_points=1.0, side_policy="BOTH", execution_cost_model: BreakoutExecutionCostModel | None = None):
    if round_trip_cost_points < 0: raise MultiTimeframeStudyError("round_trip_cost_points must be greater than or equal to 0")
    try: BreakoutReplayConfig(round_trip_cost_points=round_trip_cost_points,side_policy=side_policy,execution_cost_model=execution_cost_model)
    except ValueError as exc: raise MultiTimeframeStudyError(str(exc)) from exc
    ...
    try:
        ...
        for source in paths:
            ...
            for timeframe,lookback in TIMEFRAMES.items():
                ...
                bars=source_bars if timeframe==1 else tuple(load_ohlcv_csv(target)); replay_config=BreakoutReplayConfig(lookback,round_trip_cost_points,side_policy=side_policy,execution_cost_model=execution_cost_model); replay=replay_breakout(bars,replay_config); trades=replay["trades"]
                ...
                rows.append({"input_filename":source.name,"input_sha256":source_hash,"timeframe_minutes":timeframe,"lookback_bars":lookback,"side_policy":side_policy,"input_bar_count":len(source_bars),"resampled_bar_count":len(bars),"dropped_incomplete_bucket_count":dropped,"boundary_forced_close_count":boundary_forced_close_count,"cost_mode":"detailed" if execution_cost_model else "legacy_all_in","effective_round_trip_cost_points":replay_config.effective_round_trip_cost_points,"cost_breakdown":replay_config.execution_cost_model.serialize() if replay_config.execution_cost_model else {"cost_mode":"legacy_all_in","legacy_round_trip_cost_points":replay_config.round_trip_cost_points,"total_round_trip_cost_points":replay_config.round_trip_cost_points},"trades":[_trade(t,replay_config) for t in trades],"by_side":by_side,"total_trades":metrics["total_trades"],"gross_total_pnl_points":metrics["gross_total_pnl_points"],"net_total_pnl_points":metrics["net_total_pnl_points"],"net_profit_factor":metrics["net_profit_factor"],"net_average_trade_points":metrics["net_average_trade_points"],"net_closed_equity_drawdown_points":metrics["net_closed_equity_drawdown_points"],"long_trades":len(longs),"long_net_pnl_points":sum(t.net_pnl_points for t in longs),"short_trades":len(shorts),"short_net_pnl_points":sum(t.net_pnl_points for t in shorts),"first_market_timestamp":bars[0].timestamp.isoformat(),"last_market_timestamp":bars[-1].timestamp.isoformat(),"run_id":hashlib.sha256(deterministic_json({"source":source_hash,"timeframe":timeframe,"lookback":lookback,"cost":_cost_config(replay_config)}).encode()).hexdigest()[:20]})
        top_config=BreakoutReplayConfig(round_trip_cost_points=round_trip_cost_points,side_policy=side_policy,execution_cost_model=execution_cost_model); config={"timeframes":TIMEFRAMES,**_cost_config(top_config)}; run=hashlib.sha256(json.dumps({"inputs":[source_hashes[p] for p in paths],**config},sort_keys=True).encode()).hexdigest()
        ...
        summary={"schema_version":"1.3","configuration":config,"multi_source_aggregation":aggregation_policy,"by_timeframe":{str(tf):_timeframe_aggregate((r for r in rows if r["timeframe_minutes"]==tf),tuple(trades_by_timeframe[tf])) for tf in TIMEFRAMES}}
        manifest={"schema_version":"1.3","run_id":f"multitimeframe-{run[:16]}","input_filenames":[p.name for p in paths],"configuration":config,"configuration_sha256":hashlib.sha256(deterministic_json(config).encode()).hexdigest(),"multi_source_aggregation":aggregation_policy,"status":"completed","generated_files":["manifest.json","report.md","results.json","summary.json"],"warnings":["descriptive multi-timeframe study; no automatic selection or optimization","multi-source metrics are path-dependent descriptive aggregates, not a rollover-adjusted continuous equity curve"]}
        report="# Multi-Timeframe Breakout Study\n\n"+"\n".join(["- Offline descriptive study; no timeframe is selected automatically.",f"- Inputs: {', '.join(p.name for p in paths)}",*_cost_report_lines(config),f"- Side policy: {side_policy}"])+"\n"
...
def _cost_config(config):
    row={"round_trip_cost_points":config.round_trip_cost_points,"effective_round_trip_cost_points":config.effective_round_trip_cost_points,"cost_mode":"detailed" if config.execution_cost_model else "legacy_all_in","side_policy":config.side_policy}
